    ...
    <a-modal v-model:visible="modal.isOpen" :title="$t(modal.title)" width="60%">
      ...
      >
        ...
        <a-form-item :label="$t('require_login')" name="require_login">
          <a-switch
            v-model:checked="modal.data.require_login"
            :unCheckedValue="0"
            :checkedValue="1"
        ...
        <a-form-item :label="$t('published')" name="published">
          ...
          ><br />
          <span v-if="modal.data.entries_count > 0">
        ...
        <a-form-item
          ...
          v-if="modal.data.published"
        >
          <a-switch
            v-model:checked="modal.data.with_attendance"
            :unCheckedValue="0"
            :checkedValue="1"
          />
        ...
        <a-form-item :label="$t('banner_image')" name="banner_image">
          <div v-if="modal.data.media.length">
            <inertia-link
              :href="route('manage.form.deleteMedia', modal.data.media[0].id)"
    ...
    updateRecord() {
      this.$refs.modalRef
        ...
        .then(() => {
          ...
          this.$inertia.post(
            route("manage.forms.update", this.modal.data.id),
            this.modal.data,
            {
              onSuccess: (page) => {
                this.modal.isOpen = false;
