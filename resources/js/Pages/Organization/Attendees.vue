<template>
  <OrganizationLayout title="Dashboard">
    ...
    <div class="flex-auto pb-3 text-right">
      <a-button @click="addAttendees">{{ $t("add_attendees") }}</a-button>
    </div>
    <a-table :dataSource="attendees" :columns="columns">
      ...
      <template #bodyCell="{ column, text, record, index }">
        <template v-if="column.dataIndex == 'operation'">
          <a-button @click="deleteAttendee(record)">{{ $t("delete") }}</a-button>
        </template>
        <template v-else-if="column.dataIndex == 'member'">
          <template v-if="record.member">
            {{ $t("member") }}:{{ record.member.member_number }}
          </template>
        ...
        <template v-else-if="column.dataIndex == 'display_name'">
          <div class="editable-cell">
            <div v-if="editableData[record.id]" class="editable-cell-input-wrapper">
              <a-input
                v-model:value="editableData[record.id].display_name"
                @pressEnter="save(record.key)"
              />
              <check-outlined
                class="editable-cell-icon-check"
                @click="saveAttendee(record)"
              />
            </div>
        ...
        <template v-else-if="column.dataIndex == 'status'">
          <a-select
            ...
            style="width: 100px"
            @change="updateAttendeeAttendance(record)"
          ></a-select>
        </template>
        <template v-else>
    ...
    <a-modal v-model:visible="modal.isOpen" :title="modal.title" width="60%">
      <a-collapse v-model:activeKey="activeKey" accordion>
        <a-collapse-panel key="1" :header="$t('select_members_to_attendees_list')">
          <a-transfer
            v-model:target-keys="targetKeys"
            v-model:selected-keys="selectedKeys"
            :data-source="
              members.map((m) => ({ key: m.id.toString(), title: m.display_name }))
            "
            show-search
            ...
            :titles="[$t('members'), $t('attendees')]"
            :render="(item) => `${item.key}-${item.title}`"
            @change="handleChange"
          />
          <a-button @click="onClickShowSelected">{{
            $t("add_members_as_attendees")
          }}</a-button>
        </a-collapse-panel>
        <a-collapse-panel key="2" :header="$t('input_non_member_to_attendees_list')">
          <ol class="list-disc ml-5">
            <li v-for="(name, id) in inputNameList">
              {{ name.display_name }}
              <span class="float-right text-red-300" @click="deleteFromInputNameList(id)"
                ><DeleteOutlined
              /></span>
            </li>
          </ol>
          ...
          <a-button @click="addInputName">{{ $t("add") }}</a-button>
          <a-button @click="storeInputNameList" class="float-right">{{
            $t("save")
          }}</a-button>
        </a-collapse-panel>
        <a-collapse-panel
          key="3"
          :header="$t('batch_input_non_member_to_attendees_list')"
        >
          <ol class="list-disc ml-5">
          ...
          <a-button @click="addBatchInputNames">{{ $t("add") }}</a-button>
          <a-button @click="storeInputNameList" class="float-right">{{
            $t("save")
          }}</a-button>
        </a-collapse-panel>
...
export default {
  ...
  methods: {
    addAttendees() {
      this.modal.title = this.$t("add_attendees");
    ...
    onClickShowSelected() {
      ...
      this.$inertia.post(
        route("manage.event.attendees.store", this.event.id),
        { model: "member", data: data },
        {
          onSuccess: (page) => {
            this.modal.data = {};
    ...
    storeInputNameList() {
      this.$inertia.post(
        route("manage.event.attendees.store", this.event.id),
        { model: "other", data: this.inputNameList },
        {
          onSuccess: (page) => {
            this.modal.data = {};
            this.modal.isOpen = false;
    ...
    deleteAttendee(attendee) {
      this.$inertia.delete(
        route("manage.event.attendees.destroy", {
          event: this.event.id,
    ...
    saveAttendee(attendee) {
      this.$inertia.patch(
        route("manage.event.attendees.update", {
          event: this.event.id,
          attendee: attendee.id,
        }),
        attendee,
