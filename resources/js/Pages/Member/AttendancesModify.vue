    ...
    <a-table :dataSource="event.attendances" :columns="columns">
      ...
      <template #bodyCell="{ column, text, record, index }">
        ...
        <template v-else-if="column.dataIndex == 'status'">
          ...
          >
            <a-radio-button value="ATT">{{ $t("attend") }}</a-radio-button>
            <a-radio-button value="lATE">{{ $t("be_late") }}</a-radio-button>
            <a-radio-button value="EARLY">{{ $t("leave_early") }}</a-radio-button>
          </a-radio-group>
        </template>
        <template v-else>
